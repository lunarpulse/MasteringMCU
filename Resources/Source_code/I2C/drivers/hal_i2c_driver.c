...
void hal_i2c_master_rx(i2c_handle_t *handle, uint8_t slave_addr, uint8_t *buffer, uint32_t len)
{
	...
	hal_i2c_send_addr_first(handle->Instance,((uint8_t)(slave_addr|I2C_OAR1_ADD0)));
	while ( ! i2c_wait_untill_addr_set(handle->Instance) );

	if(handle->XferSize == 1){
		handle->Instance->CR1 &= ~I2C_CR1_ACK;
		//handle->Instance->CR1 |= I2C_REG_CR1_STOP_GEN; //generate stop after 1 byte transfer and RXNE flag on

	}else if(handle->XferSize == 2){
		handle->Instance->CR1 |= I2C_CR1_POS;
		handle->Instance->CR1 &= ~I2C_CR1_ACK;
	}
	else{
	...
	clear_addr_flag(handle->Instance); // set I2C ready status from wait status
	//for 1 byte slave will send 1 byte to master shift register, wait for RXNE interrupt to get it.
